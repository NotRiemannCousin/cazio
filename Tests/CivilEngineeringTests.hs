...
testeDeflexaoVigaAco =
    let carga = 10000 -- 10 kN
        ...
        inercia = 0.001 -- 0.001 m⁴deflexao = deflexaoViga carga comprimento modulo inercia
...
testeDeflexaoSemCarga =
    let carga = 0
            comprimento = 4
        modulo = 30e9
        inercia = 0.002deflexao = deflexaoViga carga comprimento modulo inercia
...
testeCargaCriticaBasica =
    let modulo = 200e9 -- 200 GPa (aço)
        ...
        comprimento = 3 -- 3 mcargaCritica = cargaCriticaEuler modulo inercia comprimento
...
testeCargaCriticaInfluenciaInercia =
    let modulo = 200e9
        ...
        razao = carga2 / carga1
    in abs (razao- 4.0) < 0.01testeCargaCriticaPilarCurto::Bool
...
testeCargaCriticaValorPequeno =
    let modulo = 10e9 -- material flexível
...
testeVolumeConcretoRetangulo =
    let viga = Paralelepipedo 10 0.30 0.50 -- 10m x 30cm x 50cm
        ...
        esperado = 10 * 0.30 * 0.50 -- 1.5 m³in abs (volume- esperado) < 0.001
...
testeCasoRealPilarConcreto =
    ...
    let lado = 0.40
        ...
        inercia = momentoInerciaRetangular lado lado
        area = lado * lado
        tensao = tensaoNormal carga areacargaCritica = cargaCriticaEuler moduloConcreto inercia altura
...
executarTestesCivilEngineering = do
    ...
    putStrLn $ "Aço: " ++ show testeDeflexaoVigaAcoputStrLn $ "Madeira: " ++ show testeDeflexaoVigaMadeira
