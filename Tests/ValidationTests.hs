...
testeValidarProjetoCompleto =
    let material1 = Material "Concreto" 2400 30e6 350 100
        material2 = Material "Aço" 7850 500e6 4.50 50
...
testeGerarRelatorioProjetoSemDataFim =
    ...
    in length relatorio >= 5 &&
        ...
        any (isInfixOf "Não definida") relatorio
    where
        isInfixOf xs ys = any (isPrefixOf xs) (tails ys)
        isPrefixOf [] _ = True
        isPrefixOf _ [] = False
...
testeGerarRelatorioProjetoComCalculos =
    let calculos = [ CalculoMat "Área Total" [100, 200] 20000 "m²" "A = l * w"
        ,
...
testeGerarRelatorioProjetoComFuncoes =
    let funcoes = [ Funcao (Linear 2 3) "Função linear: f(x) = 2x + 3"
        ,
...
testeEstatisticasBasicasPequenos =
    ...
    in abs (media- mediaEsperada) < 0.0001 &&
        abs (maximo- 0.005) < 0.0001 &&abs (minimo- 0.001) < 0.0001
...
testeContarPorTipoVazio =
    let projetos = []
        resultado = contarPorTipo projetosin null resultado
...
testeContarPorTipoMuitosProjetos =
    ...
    in totalContado == 100 && length resultado <= 4 -- máximo 4 tipos
    where
        tiposPorId i
        | i `mod` 4 == 1 = Civil
        | i `mod` 4 == 2 = Mecanica
...
testeProjetosEmAtrasoListaVazia =
    let projetos = []dataAtual = fromGregorian 2025 6 1
        resultado = projetosEmAtraso projetos dataAtual
    in null resultado
...
testeProjetosEmAtrasoProjetosConcluidos =
    let projetos = [ Projeto 1 "Concluído no Prazo" Civil Concluido [] [] [] [] []
        ...
        dataAtual = fromGregorian 2025 6 1
        resultado = projetosEmAtraso projetos dataAtual
    in length resultado == 1 && idProjeto (head resultado) == 3 -- só o em andamento
...
testeProjetosEmAtrasoOrdenacao =
    ...
    in length resultado == 3 && idsResultado == [3, 1, 2] -- ordem original ou específica
...
executarTestesEspecialista5 = do
    ...
    putStrLn "(Validação, Relatórios e Interface)"
    putStrLn "======================================"
    putStrLn "\n -- TESTES VALIDAR PROJETO --"
    putStrLn $ "Projeto válido: " ++ show testeValidarProjetoValido
    putStrLn $ "Nome vazio: " ++ show testeValidarProjetoNomeVazio
    putStrLn $ "Orçamento negativo: " ++ show testeValidarProjetoOrcamentoNegativo
    putStrLn $ "Sem materiais: " ++ show testeValidarProjetoSemMateriais
    putStrLn $ "Data fim inválida: " ++ show testeValidarProjetoDataInvalidaFim
    putStrLn $ "ID negativo: " ++ show testeValidarProjetoIdNegativo
    putStrLn $ "Projeto completo: " ++ show testeValidarProjetoCompleto
    putStrLn $ "Material inválido: " ++ show testeValidarProjetoMaterialInvalido
    putStrLn $ "Orçamento zero: " ++ show testeValidarProjetoOrcamentoZero
    putStrLn $ "Nome muito longo: " ++ show testeValidarProjetoNomeMuitoLongo
    putStrLn "\n -- TESTES CALCULAR CUSTO TOTAL --"
    putStrLn $ "Básico: " ++ show testeCalcularCustoTotalBasico
    ...
    putStrLn $ "Precisão: " ++ show testeCalcularCustoTotalPrecisao
    putStrLn "\n -- TESTES GERAR RELATORIO --"
    putStrLn $ "Completo: " ++ show testeGerarRelatorioProjetoCompleto
    putStrLn $ "Mínimo: " ++ show testeGerarRelatorioProjetoMinimo
    putStrLn $ "Sem data fim: " ++ show testeGerarRelatorioProjetoSemDataFim
    putStrLn $ "Concluído: " ++ show testeGerarRelatorioProjetoConcluido
    putStrLn $ "Muitos materiais: " ++ show testeGerarRelatorioProjetoMuitosMateriais
    putStrLn $ "Com cálculos: " ++ show testeGerarRelatorioProjetoComCalculos
    putStrLn $ "Orçamento alto: " ++ show testeGerarRelatorioProjetoOrcamentoAltoputStrLn $ "Tipos variados: " ++ show testeGerarRelatorioProjetoTiposVariados
    putStrLn $ "Formatação: " ++ show testeGerarRelatorioProjetoFormatacao
    putStrLn $ "Com funções: " ++ show testeGerarRelatorioProjetoComFuncoes
    ...
    putStrLn $ "Orçamento diferente: " ++ show testeCompararProjetosOrcamentoDiferente
    ...
    putStrLn $ "Formatação: " ++ show testeCompararProjetosFormatacao
    putStrLn "\n -- TESTES ESTATÍSTICAS BÁSICAS --"
    ...
    putStrLn $ "Ordem aleatória: " ++ show testeEstatisticasBasicasOrdemAleatoria
    ...
    putStrLn $ "Único: " ++ show testeContarPorTipoUnico
    putStrLn $ "Vário: " ++ show testeContarPorTipoVario
    ...
    putStrLn $ "Prazo amanhã: " ++ show testeProjetosEmAtrasoPrazoAmanha
    putStrLn $ "Projetos concluídos: " ++ show testeProjetosEmAtrasoProjetosConcluidos
    putStrLn $ "Ordenação: " ++ show testeProjetosEmAtrasoOrdenacao
