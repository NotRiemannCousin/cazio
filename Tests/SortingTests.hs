...
-- Listas com padrões específicos
...
-- Para testar estabilidade, usamos tuplas onde o primeiro elemento é chave
...
testeInsertionSortStrings =
    insertionSort ["echo", "alpha", "delta", "beta", "charlie"] == ["alpha", "beta", "charlie", "delta", "echo"]
...
-- Casos de melhor/pior cenário para insertion sort
...
-- Lista com muitas duplicatas
testeConsistenciaGrande::Bool
testeConsistenciaGrande =
    let lista = [i `mod` 50 | i <- [1..100]]
        quick = quickSort lista
        merge = mergeSort lista
        insertion = insertionSort lista
    in quick == merge && merge == insertion
