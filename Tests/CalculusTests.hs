...
testeRaizQuadratica =
    let funcao = Funcao (Quadratica 1 0 (-4)) "f(x) = x²- 4" -- raízes em ±2
...
testeSemRaizes::Bool
testeSemRaizes =
    let funcao = Funcao (Quadratica 1 0 1) "f(x) = x² + 1" -- sem raízes reais
...
testeRaizConstante =
    let funcao = Funcao (Linear 0 5) "f(x) = 5" -- função constante, sem raízes
...
testeMaximoParabola =
    let funcao = Funcao (Quadratica (-1) 0 4) "f(x) =-x² + 4" -- máximo em x=0
        maximo = encontrarMaximo funcao (-2) 2
    in case maximo of
        Just x  -> abs x < 0.1 -- máximo próximo de x=0
        Nothing -> False

testeMaximoLinearCrescente::Bool
testeMaximoLinearCrescente =
        let funcao = Funcao (Linear 1 0) "f(x) = x" -- função crescente
            maximo = encontrarMaximo funcao 0 5
    in case maximo of
        Just x  -> abs (x- 5.0) < 0.1 -- máximo no limite superior
        Nothing -> False
...
testeMaximoLinearDecrescente =
    let funcao = Funcao (Linear (-1) 0) "f(x) =-x" -- função decrescente
        maximo = encontrarMaximo funcao 0 5
    in case maximo of
        Just x  -> abs x < 0.1 -- máximo no limite inferior
        Nothing -> False
...
testeMaximoConstante =
    let funcao = Funcao (Linear 0 5) "f(x) = 5" -- função constante
        maximo = encontrarMaximo funcao 0 5
    in case maximo of
        Just x  -> x >= 0 && x <= 5 -- qualquer ponto é máximo
        Nothing -> False
...
testeMaximoSeno =
    ...
    in case maximo of
        Just x  -> abs (x- pi/2) < 0.1 -- máximo em π/2
        Nothing -> False 
...
testeMinimoParabola =
    let funcao = Funcao (Quadratica 1 0 0) "f(x) = x²" -- mínimo em x=0
        minimo = encontrarMinimo funcao (-2) 2
    in case minimo of
        Just x  -> abs x < 0.1 -- mínimo próximo de x=0
        Nothing -> False
...
testeMinimoLinearCrescente =
    ...
    in case minimo of
        Just x  -> abs x < 0.1 -- mínimo no limite inferior
        Nothing -> False
...
testeMinimoParabolaDeslocada =
    let funcao = Funcao (Quadratica 1 (-4) 4) "f(x) = (x-2)²" -- mínimo em x=2
...
testeMinimoFuncaoInvertida =
    let funcao = Funcao (Quadratica (-1) 0 0) "f(x) =-x²" -- máximo em x=0, não mínimo global
        minimo = encontrarMinimo funcao (-2) 2
    in case minimo of
        Just x  -> abs x > 1.5 -- mínimo nos extremos
        Nothing -> False
...
testeMinimoConstante =
    ...
    in case minimo of
        Just x  -> x >=(-1) && x <= 1 -- qualquer ponto é mínimo
        Nothing -> False 
...
testeComprimentoRetaHorizontal =
    ...
    in abs (comprimento- 10.0) < 0.1 -- comprimento = diferença em x
...
testeComprimentoRetaInclinada =
    let funcao = Funcao (Linear 1 0) "f(x) = x" -- reta y=x
        ...
        esperado = sqrt 2 -- hipotenusa do triângulo (1,1)
...
testeComprimentoParabola =
    let funcao = Funcao (Quadratica 1 0 0) "f(x) = x²"
        comprimento = calcularComprimentoCurva funcao 0 1
    in comprimento > 1.0 && comprimento < 2.0
...
executarTestesCalculo = do
    ...
    putStrLn "\n -- TESTES AVALIAÇÃO DE FUNÇÕES --"
    putStrLn $ "Linear: " ++ show testeAvaliacaoLinear
    putStrLn $ "Linear Zero: " ++ show testeAvaliacaoLinearZero
    putStrLn $ "Linear Negativo: " ++ show testeAvaliacaoLinearNegativo
    putStrLn $ "Quadrática: " ++ show testeAvaliacaoQuadratica
    putStrLn $ "Quadrática Completa: " ++ show testeAvaliacaoQuadraticaCompleta
    putStrLn $ "Exponencial: " ++ show testeAvaliacaoExponencial
    putStrLn $ "Exponencial Escalada: " ++ show testeAvaliacaoExponencialEscalada
    putStrLn $ "Logarítmica: " ++ show testeAvaliacaoLogaritmica
    putStrLn $ "Logarítmica Escalada: " ++ show testeAvaliacaoLogaritmicaEscalada
    putStrLn $ "Trigonométrica: " ++ show testeAvaliacaoTrigonometrica
    
    putStrLn "\n -- TESTES DERIVADAS --"
    putStrLn $ "Linear: " ++ show testeDerivadaLinear
    putStrLn $ "Constante: " ++ show testeDerivadaConstante
    putStrLn $ "Quadrática: " ++ show testeDerivadaQuadratica
    putStrLn $ "Seno: " ++ show testeDerivadaSeno
    
    putStrLn "\n -- TESTES INTEGRAIS --"
    putStrLn $ "Linear: " ++ show testeIntegralLinear
    putStrLn $ "Constante: " ++ show testeIntegralConstante
    putStrLn $ "Zero: " ++ show testeIntegralZero
    putStrLn $ "Simétrica: " ++ show testeIntegralSimetrica
    
    putStrLn "\n -- TESTES RAÍZES --"
    putStrLn $ "Linear: " ++ show testeRaizLinear
    putStrLn $ "Quadrática: " ++ show testeRaizQuadratica
    putStrLn $ "Sem Raízes: " ++ show testeSemRaizes
    putStrLn $ "Raiz Dupla: " ++ show testeRaizDupla
    putStrLn $ "Raiz Constante: " ++ show testeRaizConstante
    
    putStrLn "\n -- TESTES MÁXIMOS --"
    putStrLn $ "Parábola: " ++ show testeMaximoParabola
    putStrLn $ "Linear Crescente: " ++ show testeMaximoLinearCrescente
    putStrLn $ "Linear Decrescente: " ++ show testeMaximoLinearDecrescente
    putStrLn $ "Constante: " ++ show testeMaximoConstante
    putStrLn $ "Seno: " ++ show testeMaximoSeno
    
    putStrLn "\n -- TESTES MÍNIMOS --"
    putStrLn $ "Parábola: " ++ show testeMinimoParabola
    putStrLn $ "Linear Crescente: " ++ show testeMinimoLinearCrescente
    putStrLn $ "Parábola Deslocada: " ++ show testeMinimoParabolaDeslocada
    putStrLn $ "Função Invertida: " ++ show testeMinimoFuncaoInvertida
    putStrLn $ "Constante: " ++ show testeMinimoConstante
    
    putStrLn "\n -- TESTES COMPRIMENTO DE CURVA --"
    putStrLn $ "Reta Horizontal: " ++ show testeComprimentoRetaHorizontal
    putStrLn $ "Reta Inclinada: " ++ show testeComprimentoRetaInclinada
    putStrLn $ "Reta Vertical: " ++ show testeComprimentoRetaVertical
    putStrLn $ "Constante: " ++ show testeComprimentoConstante

testesCalculo :: [(String, Bool)]
testesCalculo = [
        -- Avaliação de Funções (10 testes)
        ("Avaliação Linear", testeAvaliacaoLinear),
        ("Avaliação Linear Zero", testeAvaliacaoLinearZero),
        ("Avaliação Linear Negativo", testeAvaliacaoLinearNegativo),
        ("Avaliação Quadrática", testeAvaliacaoQuadratica),
        ("Avaliação Quadrática Completa", testeAvaliacaoQuadraticaCompleta),
        ("Avaliação Exponencial", testeAvaliacaoExponencial),
        ("Avaliação Exponencial Escalada", testeAvaliacaoExponencialEscalada),
        ("Avaliação Logarítmica", testeAvaliacaoLogaritmica),
        ("Avaliação Logarítmica Escalada", testeAvaliacaoLogaritmicaEscalada),
        ("Avaliação Trigonométrica", testeAvaliacaoTrigonometrica),
        
        -- Derivadas (4 testes)
        ("Derivada Linear", testeDerivadaLinear),
        ("Derivada Constante", testeDerivadaConstante),
        ("Derivada Quadrática", testeDerivadaQuadratica),
        ("Derivada Seno", testeDerivadaSeno),
        
        -- Integrais (4 testes)
        ("Integral Linear", testeIntegralLinear),
        ("Integral Constante", testeIntegralConstante),
        ("Integral Zero", testeIntegralZero),
        ("Integral Simétrica", testeIntegralSimetrica),
        
        -- Raízes (5 testes)
        ("Raiz Linear", testeRaizLinear),
        ("Raiz Quadrática", testeRaizQuadratica),
        ("Sem Raízes", testeSemRaizes),
        ("Raiz Dupla", testeRaizDupla),
        ("Raiz Constante", testeRaizConstante),
        
        -- Máximos (5 testes)
        ("Máximo Parábola", testeMaximoParabola),
        ("Máximo Linear Crescente", testeMaximoLinearCrescente),
        ("Máximo Linear Decrescente", testeMaximoLinearDecrescente),
        ("Máximo Constante", testeMaximoConstante),
        ("Máximo Seno", testeMaximoSeno),
        
        -- Mínimos (5 testes)
        ("Mínimo Parábola", testeMinimoParabola),
        ("Mínimo Linear Crescente", testeMinimoLinearCrescente),
        ("Mínimo Parábola Deslocada", testeMinimoParabolaDeslocada),
        ("Mínimo Função Invertida", testeMinimoFuncaoInvertida),
        ("Mínimo Constante", testeMinimoConstante),
        
        -- Comprimento de Curva (4 testes)
        ("Comprimento Reta Horizontal", testeComprimentoRetaHorizontal),
        ("Comprimento Reta Inclinada", testeComprimentoRetaInclinada),
        ("Comprimento Reta Vertical", testeComprimentoRetaVertical),
        ("Comprimento Constante", testeComprimentoConstante)
    ]